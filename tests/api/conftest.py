...
def api_client() -> BookingClient:
    ...
    # Retry auth up to 5 times — Heroku app sleeps and needs a wake-up call
    for attempt in range(5):
        ...
        if resp.status_code == 200 and "token" in resp.json():
            ...
            if token and token != "Bad credentials":
                ...
                break
        print(f"Auth attempt {attempt + 1} failed, retrying in 3s...")
        time.sleep(3)
    else:
        pytest.fail("Could not authenticate after 5 attempts. Heroku app may be down.")
